...
// Function to fetch farmer details asynchronously
const fetchFarmer = async (orderId) => {
   try {
      // Fetching farmer details based on order ID
      const res = await fetch(
         `${process.env.NEXT_PUBLIC_SITE_URL}/api/users?id=${orderId}`, // Endpoint for fetching farmer details
         { cache: "no-cache" } // Setting cache control
      );
      // Parsing the fetched data as JSON
      const order = await res.json();
      // Returning the farmer details
      return order;
   } catch (error) {
